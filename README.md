...
Menu-controlled preferences are stored in `~/.config/codex-session-meter/settings.toml`.
...
6. Verify local config/cache directories are not committed: `~/.config/codex-session-meter/` and `~/.cache/codex-session-meter/`.

## Manual Verification Checklist

1. Install extension with `./install.sh`.
2. Start with no auth and verify `bejelentkezés kell` or `adatforrás kell` appears.
3. Run `codex-session-meter login` or `codex login`.
4. Run `codex-session-meter refresh --json` and verify `source_label` is `Codex CLI API` and reset time appears.
...
This removes the extension, helper virtualenv, and command symlink. It keeps config/cache files so debug samples are not deleted unexpectedly. Run `codex-session-meter logout` before uninstalling if you want to log out of Codex CLI.
